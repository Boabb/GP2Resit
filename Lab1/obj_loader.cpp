...
IndexedModel OBJModel::ToIndexedModel()
{
    ...
    for(unsigned int i = 0; i < numIndices; i++)
    {
        ...
        if(hasUVs && currentIndex->uvIndex < uvs.size() && currentIndex->uvIndex >= 0)
            currentTexCoord = uvs[currentIndex->uvIndex];
        else
            currentTexCoord = glm::vec2(0,0);
        if(hasNormals && currentIndex->normalIndex < normals.size() && currentIndex->normalIndex >= 0)
    ...
    // Ensure texCoords and normals match positions size
    if (result.texCoords.size() < result.positions.size()) {
        result.texCoords.resize(result.positions.size(), glm::vec2(0, 0));
    }
    if (result.normals.size() < result.positions.size()) {
        result.normals.resize(result.positions.size(), glm::vec3(0, 0, 1));
    }
